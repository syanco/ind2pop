...
maples <- read.csv("data/sugar_maple_samp.csv") %>% 
  rename(lng = LON,
         lat = LAT) %>% 
  mutate(date = as_date(timestamp)) %>% 
  filter(!is.na(lng),
...
# pull pre-annotations from MOL, filter to movement bounding box + 1 degree
gad_mol <- get_species_scenario('Anas strepera', 'landsat8', 'evi', 100, 16) %>% 
  # filter(latitude >= (bb_ele$minlat-1) & latitude <= (bb_ele$maxlat+1),
  # longitude >= (bb_ele$minlon-1) & longitude <= (bb_ele$maxlon+1)) %>% 
  rename(lng = longitude,
         lat = latitude,
...
#
# pull pre-annotations from MOL, filter to movement bounding box + 1 degree
gad_mol <- get_species_scenario('Acer saccharum', 'landsat8', 'evi', 100, 16) %>% 
  # filter(latitude >= (bb_ele$minlat-1) & latitude <= (bb_ele$maxlat+1),
  # longitude >= (bb_ele$minlon-1) & longitude <= (bb_ele$maxlon+1)) %>% 
  rename(lng = longitude,
         lat = latitude,
