...
df_ele <- data.frame(mu = c(mix_mean_ele, vc_mean_ele, pop_mean_ele),
                     ...
                     mu_ci_l = c(ele_mean_ci[1], vc_mean_CI_ele[1], pop_mean_ele_CI[1]),
                     mu_ci_h = c(ele_mean_ci[2], vc_mean_CI_ele[2], pop_mean_ele_CI[2]),
                     var_ci_l = c(ele_var_ci[1], vc_ele_ci[1], pop_var_ele_CI[1]), 
                     var_ci_h = c(ele_var_ci[2], vc_ele_ci[2], pop_var_ele_CI[2]))
...
(ele_mu_comp <-df_ele %>% 
    ...
    ylab("Mean") +
    # scale_y_continuous(breaks = c(304, 305, 306)) +
    coord_flip())
ggsave(filename = "out/figs/ele_mu_com.png", ele_mu_comp, width = 2.6, height = 1.5, units = "in")
...
(ele_var_comp <-df_ele %>% 
    ...
    coord_flip())
ggsave(filename = "out/figs/ele_var_com.png", ele_var_comp, width = 1.5, height = 1.5, units = "in")
